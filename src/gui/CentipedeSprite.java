...
public class CentipedeSprite extends EntitySprite {
	...
	protected void cacheImages(){
		...
		if (headCache == null) {
			...
			//again, the second images are nice animations, but I'm not supporting that now
			headCache.put(Direction.LEFT, head.getSubimage(0,quarter*0, half, quarter));
			headCache.put(Direction.RIGHT, head.getSubimage(0,quarter*1, half, quarter));
			headCache.put(Direction.DOWN, head.getSubimage(0,quarter*2, half, quarter));
			headCache.put(Direction.UP, head.getSubimage(0,quarter*3, half, quarter));
