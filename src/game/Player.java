...
public class Player implements Entity, Runnable {
	...
	/**
	 * This method is called by the handler for keyboard input. It should not be
	 * confused with updateLocation, which is called by the board after all
	 * movement issues have been resolved.
	 */
	...
	/**
	 * Start moving in a direction due to keyboard input. The Player will move
	 * in this direction until told otherwise.
	 */
	...
	public void fire() {
		...
		m_board.createEntity(m_location.x + BULLET_SPAWN_DX, m_location.y
				+ BULLET_SPAWN_DY, EntityTypes.BULLET);
	}

	/**
	 * Requests movement from the board based on our current velocity. Not to be
	 * confused with moveToLocation, which requests a move directly to the given
	 * location.
	 */
	public void move() {
	...
	/**
	 * Handles collisions with other entities. The player dies if it hits a
	 * centipede and stops moving if it hits a mushroom. Bullet collisions
	 * should be impossible since player movement is side-to-side. If a bullet
	 * collision happens for some reason, we do nothing and the bullet will go
	 * away.
	 */
	...
	/**
	 * If we're dead, we got hit by a centipede and need to remove ourselves
	 * from the board.
	 */
	public void die() {
	...
	/**
	 * Stops moved initiated by beginMove. Note that the player (for now) does
	 * not need to move diagonally.
	 **/
	...
	/**
	 * Allows Player to be run as its own thread. Player will move a certain
	 * distance every step based on human input.
	 */
	public void run() {
